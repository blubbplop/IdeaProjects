    ...
    public void action(String[] args, MessageReceivedEvent event) {
        ...
        if (args.length < 1) {
            event.getTextChannel( ).sendMessage(
                    error.setDescription("Bitte geben Sie eine gültige Anzahl von Nachrichten an, die Sie löschen möchten.").build( )
            ).queue( );
        ...
        if (args[0].equals("all")) {
            ...
            try {
                ...
                msgs = history.retrievePast(event.getTextChannel().getHistory().size()).complete();
                event.getTextChannel().deleteMessages(msgs).queue();

                Message msg = event.getTextChannel().sendMessage(
                        new EmbedBuilder().setColor(Color.GREEN).setDescription("Alle Nachrichten gelöscht!").build()
                ).complete();
